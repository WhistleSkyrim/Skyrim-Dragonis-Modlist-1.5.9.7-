...
1. Качаешь все архивы.     
2.  Переименовываешь архивы следующим образом:

- Удаляешь все после 7z ДО ТОЧКИ, ПОСЛЕ ТОЧКИ НИЧЕГО НЕ УДАЛЯЙ 

- И у тебя должно получиться к примеру SkyrimDragonis.7z.002 вместо SkyrimDragonis.7z-001.002

ПОСЛЕ ЭТОГО НАЖИМАЕШЬ ПРАВОЙ КНОПКОЙ ПО АРХИВУ С НОМЕРОМ .001 И НАЖИМАЕШЬ РАСПОКАВАТЬ В НУЖНЫЙ ДИСК
